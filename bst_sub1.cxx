...
/* Copyright (C) 1994/2000  by Andrew B. Kahng, C.-W. Albert Tsao    */
/***********************************************************************/
void print_Point(FILE *f, PointType pt) {
...
/********************************************************************/
void draw_a_buffer(FILE *out,int v) {
double x, y, x0,y0,x1,y1,t, t1, t2;
double d0, d1;
int cid, n,par;

  const TrrType & ctrr = gBoundedSkewTree->Ctrr() ; 
  t1 = ctrr.Width(X) /1000.0;
  t2 = ctrr.Width(Y) /1000.0;
  t =  tMAX(t1,t2);
  ...
  if (equal(EdgeLength[v],0) || equal(StubLength[v],0) ) {
    ...
    d0 = StubLength[v]/EdgeLength[v];
    d1 = 1 - d0;                           
    x = (x0*d1 + x1*d0);
    y = (y0*d1 + y1*d0);
  }
  cid = Cluster_id[Node[v].L];
  n=calc_buffer_fanout(v);

  fprintf(out,"move %f %f v:%d cid:%d n = %d\n", x+t,y+t,v, cid, n);   
  fprintf(out,"     %f %f StubLength = %f\n", x+t, y-t, StubLength[v]);   
  fprintf(out,"     %f %f \n", x-t,y-t);   
  fprintf(out,"     %f %f \n", x-t,y+t);   
  fprintf(out,"     %f %f leng=%f \n", x+t,y+t, EdgeLength[v]);   
...
void print_bst_at_level(FILE *out, int L) {
  ...
  for (unsigned i=0, j=0;i<npoints;++i) {
...
/***********************************************************************/
/* read in input  Topology                                             */
/***********************************************************************/
void read_input_topology_sub(FILE *f) {
...
char line[100];
   ...
   while ( fgets(line, 100, f) ) { 
     ...
     sscanf(line, "id:%d child:%d %d par:%d\n",
          &i, &lchild, &rchild, &par) ;
      if ( line[0]=='i' &&  line[1] == 'd' && numTokens == 4 ) {
